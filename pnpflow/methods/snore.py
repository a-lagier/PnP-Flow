...
class SNORE(object):
    ...
    def learning_rate_strat(self, lr, t):
        ...
        gamma_styles = {
            ...
            'snore': lambda lr, t: lr * (1 - t)**self.args.alpha,
        }
    ...
    def solve_ip(self, test_loader, degradation, sigma_noise, H_funcs=None):
        ...
        for batch in range(self.args.max_batch):
            ...
            use_sigma_noise = self.args.use_sigma_noise

            sigma_0 = self.args.sigma_0 * (sigma_noise if use_sigma_noise else 1.)
            alpha_0 = self.args.alpha_0 
            sigma_last = self.args.sigma_last * (sigma_noise if use_sigma_noise else 1.)
            alpha_last = self.args.alpha_last
            m = self.args.m
            max_iter = self.args.max_iter
            steps = max_iter
            sigmas_levels = torch.linspace(sigma_0, sigma_last, m)
            alphas_levels = torch.linspace(alpha_0, alpha_last, m)
            bins = torch.linspace(0, m - 1, max_iter).round().long()

            sigmas = sigmas_levels[bins]
            alphas = alphas_levels[bins]
            ...
            with torch.no_grad():
                for count, iteration in enumerate(range(int(steps))):
                    ...
                    z = x - gamma * self.grad_datafit(x, noisy_img, H, H_adj)
                    # import ipdb; ipdb.set_trace()
                    sigma = sigmas[iteration]
                    x_tilde = x + sigma * torch.randn_like(x)
                    x_tilde_rescaled = x_tilde / (1+sigma)
                    t1 = torch.ones(len(x), device=self.device)  * (1 / (1+ sigma))
                    # import ipdb; ipdb.set_trace()
                    den = self.denoiser(x_tilde_rescaled, t1)
                    # import ipdb; ipdb.set_trace()
                    
                    
                    x_new = z - (gamma*  alphas[iteration]  ) * (x - den)
                    # x_new = z - (gamma*  alphas[iteration]  ) * (x - den)
